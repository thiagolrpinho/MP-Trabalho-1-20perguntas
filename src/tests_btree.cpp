...
TEST_CASE( "Binary Tree Delete", "[binary_tree]" ) {
  ...
  SECTION( "a branch can be deleted and those below it are deleted too")
  {
    ...
    pBranch->getLeftBranch()->insertBranch("Abaixo do abaixo?");

    REQUIRE_FALSE( pBranch == nullptr );
    REQUIRE_FALSE( pBranch->getLeftBranch() == nullptr );
    REQUIRE_FALSE( pBranch->getLeftBranch()->getLeftBranch() == nullptr );
    REQUIRE( pBranch->getLeftBranch()->getLeftBranch()->getLeftBranch() == nullptr );

    pBranch->cutBranch();
    
    REQUIRE( pBranch->getLeftBranch() == nullptr );
  }
}
